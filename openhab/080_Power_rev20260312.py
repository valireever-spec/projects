...
# =========================================================
# =========================================================
# DEBUG: PRIZA ACTIVATION TRACKER (added 2026-06-30)
# =========================================================
@rule("DEBUG: Priza3/7 state & command tracker", description="Log every Priza3/7 state change and commands", tags=["DEBUG", "Priza"])
@when("Item Priza3_Power changed")
@when("Item Priza3_Power received command")
@when("Item Priza7_Power changed")
@when("Item Priza7_Power received command")
def debug_priza_all(event):
    """Log Priza3/7 state changes AND commands with full power/intent context."""
    item_name = event.itemName

    # Determine if this is a state change or command
    if hasattr(event, 'itemState'):
        # State change event
        new_state = str(event.itemState)
        old_state = str(event.oldItemState)
        event_type = "STATE"
    else:
        # Command event
        new_state = str(event.itemCommand)
        old_state = "N/A"
        event_type = "CMD"

    pwr_item = str(items["PWRConsumption"]) if items["PWRConsumption"] not in [NULL, UNDEF] else "UNKNOWN"
    house_power = safe_bd("House_Power_Consumption")

    msg = "DEBUG_PRIZA [{}] {} {} → {} | PWR={} DIR={} INTENT={} HOUSE_PWR={}".format(
        event_type,
        item_name, old_state, new_state,
        pwr_item,
        direction_state,
        intent_state,
        house_power
